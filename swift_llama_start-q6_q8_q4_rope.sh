...
# Fit E172/E173 (2026-09-16, q8_0 K / q4_0 V, Swift model): -ts
# 34,22,44 kept, NO -ot. -c 393500 (n_ctx 393984, slots 2 x 196992,
# frees ~990/~350/~200). E172 first shipped 400300 (ROCm1 free 102) -
# REVOKED: same fattn-pool OOM class as E173 found on rerun (device 1,
# ggml-cuda.cu:507, probe-225449). 393984 passed the full check twice
# (chk-q4-393k-r1/r2: acc .686, TG 1024 clean, temp-0 repro match).
# 405504 fits at startup but ROCm1 free 22; 420000 no-ot dies on the
# TRANSIENT first-reserve compute peak (1857 MiB, grows with C).
# ot3 variant ceiling ~424k (430000 listened at 6/381/62) - parked.
# Speed test at 340k (lane swift340): pp16384 328.3 / tg 20.8 /
# acc .686 / repro_ok. NOTE swift q8/q8 PP is ~5% faster (345.8) -
# q4_0 V pays the force_convert dequant on gfx906.
# --pipeline-parallel off: see E173 - forced "on" + -ot crashes; the
...
exec "$BIN" \
  ...
  -b 16384 -ub 256 --ctx-checkpoints 30 \
  ...
  -ts 34,23,43 -sm layer -c 419430 \
  --no-mmproj-offload \
  --log-file "$LOG_DIR/llama-server-iq6v-f16-f16.log" \
  --log-prompts-dir "$LOG_DIR/prompts" \
  --rope-scaling yarn --rope-scale 1.6 --override-kv qwen35.context_length=int:419430  --override-kv qwen35.rope.scaling.original_context_length=int:262144
