#!/usr/bin/env bash
# PPL A/B: upstream master (~/dev/llama.cpp/build-stock) vs this fork
# (build-dflash-novega), same model as 2llama-start-iq6v-dflash2-v23.sh.
#
# Base model only (no mmproj, no drafter): PPL exercises the eval path
# the server uses, without spec decoding.
#
# Flag choices:
# - production tensor split 35,20,45 + -sm layer + -fa on, same as v23
# - default f16 KV (no -ctk/-ctv) to isolate weight-matmul numerics,
#   consistent with the dualacc gate (E92)
# - -ngl 99 offloads the full main model (clamps to n_layer)
# - stop the production server first: the 27B model does not fit
#   alongside it. ~10 min per side.
set -euo pipefail

cd "$(dirname "$0")"
ROOT=$(cd .. && pwd)

UP_ROOT=${UP_ROOT:-/home/srcds/dev/llama.cpp}
BIN_A=${BIN_A:-$ROOT/build-dflash-novega/bin/llama-perplexity}
BIN_B=${BIN_B:-$UP_ROOT/build-stock/bin/llama-perplexity}
MODEL=${MODEL:-/home/srcds/ai/ai/Qwen3.8-27B.i1-Q6_K.gguf}
TEXT=${TEXT:-/home/srcds/ai/ai/log.txt}

for b in "$BIN_A" "$BIN_B"; do
    [ -x "$b" ] || { echo "error: not found: $b" >&2; exit 1; }
done

COMMON=(
    -m "$MODEL" -f "$TEXT"
    --device rocm0,vulkan1,rocm1 -ngl 99
    -sm layer -ts 35,20,45
    -c 4096 -b 1024 -ub 384
    --threads 9 --threads-batch 10 --no-mmap -fa on
)

export HSA_OVERRIDE_GFX_VERSION=9.0.6 HSA_XNACK=0 HIP_VISIBLE_DEVICES=0,1
export HIP_FORCE_P2P=1 GPU_SINGLE_ALLOC_PERCENT=100 HSA_ENABLE_SDMA=1
export HSA_DISABLE_FRAGMENT_ALLOCATOR=0 GPU_MAX_ALLOC_PERCENT=100
export AMD_LOG_LEVEL=0

run() { # $1 label, $2 bin, $3 libdir
    echo "== $1 ($2)"
    LD_LIBRARY_PATH=/home/srcds/rocm-gfx906-xnack/lib:$3:/opt/rocm-6.1.0/lib \
        "$2" "${COMMON[@]}" 2>&1 | grep -E "perplexity:|llama_perf|Final estimate" | tail -6
}

run fork    "$BIN_A" "$ROOT/build-dflash-novega/bin"
echo
run upstream "$BIN_B" "$UP_ROOT/build-stock/bin"
