#!/usr/bin/env bash
# A/B for GGML_CUDA_VEGA_TUNE_MMQ_DUALACC (mmq dual accumulator chains,
# journal E92). Two trials, only BIN differs:
#   A = build-dflash-novega  (production, DUALACC off)
#   B = build-dualacc        (DUALACC on, commit 2ecfe9852)
# Lane = current UD production shape (E89): UD-Q6_K_L @ 256k, ts 40,20,40.
#
# Expectation: pp_tps up (target +15-25%); sha WILL move (reassociation,
# not a numerics bug) - the gates are acc within noise and the perplexity
# check in the journal entry. tg within noise.
#
# Stop the production server first (trials need the full 40 GB).
# ~30-40 min per trial. Fallback if it does not fit: CTX=250112.
# Results -> journal E92 (journal/JOURNAL-2026-09-02.md).
set -euo pipefail
...
BIN_B=${BIN_B:-$ROOT/build-dualacc/bin/llama-server}
...
BIN=$BIN_A MODEL=$MODEL PORT=$PORT ./ab-bench.sh dualacc-A "${EXTRAS[@]}"

echo
echo "== B: dual accumulator chains ($BIN_B)"
BIN=$BIN_B MODEL=$MODEL PORT=$PORT ./ab-bench.sh dualacc-B "${EXTRAS[@]}"
