...
void ggml_vk_load_shaders(vk_device& device, vk_pipeline requested) {
    ...
    for (auto &fa : device->pipeline_flash_attn_f32_f16) {
        ...
        ggml_vk_create_pipeline(device, fa.second, name, spv_size, spv_data, "main", 9,
...
void ggml_vk_flash_attn(ggml_backend_vk_context * ctx, vk_context& subctx, const ggml_tensor * q, const ggml_tensor * k, const ggml_tensor * v, const ggml_tensor * mask, const ggml_tensor * sinks, const ggml_tensor * nvis, ggml_tensor * dst) {
    ...
    vk_subbuffer nvis_buf = nvis ? ggml_vk_tensor_subbuffer(ctx, nvis) : q_buf;
    ...
    uint32_t mask_n_head_log2 = ((sinks != nullptr) << 24) | ((nvis != nullptr) << 25) | n_head_log2;
    ...
    if (split_k > 1) {
        ...
        ggml_vk_dispatch_pipeline(ctx, subctx, pipeline,
                                    {q_buf, k_buf, v_buf, mask_buf, sinks_buf, split_k_buf, mask_opt_buf, sparse_buf, nvis_buf},
        ...
        ggml_vk_dispatch_pipeline(ctx, subctx, pipeline,
                                    {q_buf, k_buf, v_buf, mask_buf, sinks_buf, dst_buf, mask_opt_buf, sparse_buf, nvis_buf},
...
static bool ggml_backend_vk_device_supports_op(ggml_backend_dev_t dev, const ggml_tensor * op) {
    ...
    switch (op->op) {
        ...
        case GGML_OP_FLASH_ATTN_EXT:
            {
                if (op->src[5] && (op->src[5]->type != GGML_TYPE_I32 || op->src[5]->ne[0] != op->src[0]->ne[1])) {
                    return false;
                }
