...
template <
    ggml_type type, int ncols_dst, bool has_fusion, bool small_k = false, bool halve_iters = false,
    int q8_1_layout_block_size = MMVQ_Q8_1_BLOCK_SIZE_STANDARD>
__launch_bounds__((mmvq_layout_policy<type, q8_1_layout_block_size>::nwarps(ncols_dst, get_device_table_id(), small_k, halve_iters)*ggml_cuda_get_physical_warp_size()), 1)
static __global__ void mul_mat_vec_q(
    ...
    constexpr int nwarps = layout_policy::nwarps(ncols_dst, table_id, small_k, halve_iters);
...
template<ggml_type type, int c_ncols_dst, bool small_k = false, bool halve_iters = false, int q8_1_layout_block_size = MMVQ_Q8_1_BLOCK_SIZE_STANDARD>
static void mul_mat_vec_q_switch_fusion(
    ...
    if constexpr (c_ncols_dst == 1) {
        if (has_fusion) {
            ...
            ggml_cuda_kernel_launch(mul_mat_vec_q<type, c_ncols_dst, true, small_k, halve_iters, q8_1_layout_block_size>, launch_params,
    ...
    const ggml_cuda_kernel_launch_params launch_params = ggml_cuda_kernel_launch_params(block_nums, block_dims, nbytes_shared, stream);
    ggml_cuda_kernel_launch(mul_mat_vec_q<type, c_ncols_dst, false, small_k, halve_iters, q8_1_layout_block_size>, launch_params,
        vx, vy, ids, fusion, dst, ncols_x, nchannels_y, stride_row_x, stride_col_y, stride_col_dst,
        channel_ratio, stride_channel_x, stride_channel_y, stride_channel_dst,
        sample_ratio, stride_sample_x, stride_sample_y, stride_sample_dst, ids_stride);
}
...
static void mul_mat_vec_q_switch_ncols_dst(
    ...
    switch (ncols_dst) {
        case 1: {
            ...
            // packed q8_1 layout owns the ncols_dst == 1 geometry, the small_k / halve_iters paths do not apply
            if constexpr (use_q8_1_layout) {
                const std::pair<dim3, dim3> dims = calc_launch_params<type, q8_1_layout_block_size>(c_ncols_dst, nrows_x, nchannels_dst,
                                                                      nsamples_dst, warp_size, table_id);
                mul_mat_vec_q_switch_fusion<type, c_ncols_dst, false, false, q8_1_layout_block_size>(
            ...
            const auto launch = [&](auto small_k_tag, auto halve_iters_tag) {
                ...
                mul_mat_vec_q_switch_fusion<type, c_ncols_dst, c_small_k, c_halve_iters, q8_1_layout_block_size>(
                    vx, vy, ids, fusion, dst, ncols_x, nchannels_y_fd, stride_row_x, stride_col_y, stride_col_dst,
                    channel_ratio_fd, stride_channel_x, stride_channel_y, stride_channel_dst, sample_ratio_fd,
                    stride_sample_x, stride_sample_y, stride_sample_dst, dims.first, dims.second, 0, ids_stride,
                    stream);
...
#define MMVQ_SWITCH_TYPE(type) \
        ...
        MMVQ_SWITCH_TYPE(GGML_TYPE_Q2_0);
