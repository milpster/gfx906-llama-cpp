// Chunked Gated DeltaNet forward (port from mxxm-t/mx-llama.cpp, commits
// 55b275262e + 1248e5e376 + 4405b44cd7, iacopPBK / DENEB1312).
// One thread-block owns a single (head, sequence) and loops over chunks of
// CS tokens; the recurrent state S_v x S_v stays in registers across the
// whole prefill (no HBM round-trips per token). GCN DPP warp reductions
// are compiled for gfx906 only, other archs fall back to warp_reduce_sum.
...
// Chunk size: 64 tokens per chunk for the scalar-gate path, 16 for KDA
// (per-feature gate), matching the upstream port's tuning.
