...
// fork: split-plane q8_0S set_rows, row layout = [ne00/32 fp16 d][ne00 int8 qs]
template <typename idx_t>
static __global__ void k_set_rows_q8_0s(const float * __restrict__ src0,
                                        const idx_t * __restrict__ src1,
    ...
    const float * src0_row = src0 + i01*s01 + i02*s02 + i03*s03;
    char * dst_row_ptr = dst + dst_row*s1 + i02*s2 + i03*s3;

    const int64_t ib = i00 / QK8_0;
    // fork layout: per-256-elem slice = [8 fp16 d][256 int8 qs], slice stride 272B (matches fattn head views)
    char * slice = dst_row_ptr + (ib/8)*272;
    half * d_ptr = (half *) slice;
    int8_t * qs_ptr = (int8_t *) (slice + 16);

    const float * x = src0_row + i00;

    float amax = 0.0f;
#pragma unroll
    for (int l = 0; l < QK8_0; l++) {
        amax = fmaxf(amax, fabsf(x[l]));
    }

    const float dm = amax / 127.0f;
    const float id = amax > 0.0f ? 1.0f/dm : 0.0f;
    d_ptr[ib % 8] = __float2half(dm);

    int8_t * qs = qs_ptr + (ib % 8)*QK8_0;
#pragma unroll
    for (int l = 0; l < QK8_0; l++) {
        qs[l] = (int8_t) roundf(x[l]*id);
    }
}

template <typename idx_t>
static void set_rows_cuda_q8_0s(
    ...
    if (ne_total > 0 && ne00 > 0 && ne01 > 0 && ne02 > 0 && ne11 > 0 && ne12 > 0) {
        ...
        k_set_rows_q8_0s<idx_t><<<grid_size, block_size, 0, stream>>>(
            src0_d, src1_d, dst_d, ne_total, nd, s01, s02, s03, s10, s11, s12, s1, s2, s3,
...
static void set_rows_cuda(ggml_backend_cuda_context & ctx, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    ...
    } else if (dst->type == GGML_TYPE_Q8_0S) {
        set_rows_cuda_q8_0s<idx_t>(
