...
static void test(void) {
    ...
    {
        common_params hip_fa_params;
        assert(hip_fa_params.hip_fa_force_vec);

        argv = {"binary_name", "-m", "model.gguf", "--hip-fa-force-vec", "off"};
        assert(common_params_parse(argv.size(), list_str_to_char(argv).data(), hip_fa_params, LLAMA_EXAMPLE_COMMON));
        assert(!hip_fa_params.hip_fa_force_vec);

        argv = {"binary_name", "-m", "model.gguf", "--hip-fa-force-vec", "on"};
        assert(common_params_parse(argv.size(), list_str_to_char(argv).data(), hip_fa_params, LLAMA_EXAMPLE_COMMON));
        assert(hip_fa_params.hip_fa_force_vec);

        argv = {"binary_name", "-m", "model.gguf", "--hip-fa-force-vec", "maybe"};
        assert(!common_params_parse(argv.size(), list_str_to_char(argv).data(), hip_fa_params, LLAMA_EXAMPLE_COMMON));
    }
